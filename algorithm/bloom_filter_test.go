...
func TestAddAndGet2(t *testing.T) {
	...
	stringLen := 30
	filter := NewFilter(uint64(dataSize), 3, false)
...
func TestAddAndGetSync(t *testing.T) {
	...
	filter := NewFilter(uint64(sizeData), 3, true)
...
func BenchmarkFilter_Add(b *testing.B) {
	...
	filter := NewFilter(uint64(dataTestSize), 3, false)
	b.ResetTimer()
	b.StartTimer()
